...
class PaymentController {
    ...
    static addPayment(paymentDetails, actor, distributedAmounts) {
        ...
        if (paymentDetails._type == 0) {
            distributedAmounts.payment[actor] = ethers.BigNumber.from(distributedAmounts.payment[actor].toString()).add(
                paymentDetails._payment.toString()
            );
        } else if (paymentDetails._type == 1) {
            distributedAmounts.sellerDeposit[actor] = ethers.BigNumber.from(distributedAmounts.sellerDeposit[actor].toString()).add(
                paymentDetails._payment.toString()
            );
        } else if (paymentDetails._type == 2) {
            distributedAmounts.buyerDeposit[actor] = ethers.BigNumber.from(distributedAmounts.buyerDeposit[actor].toString()).add(
                paymentDetails._payment.toString()
            );
        }
    }
