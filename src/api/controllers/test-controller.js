...
class TestController {
  ...
  async createVoucher(req, res, next) {
    ...
    const txValue = ethers.BigNumber.from(voucherSupply.price.toString()).add(
      voucherSupply.buyerDeposit.toString()
    );
    let tx;
    let metadata = {};

    try {
      tx = await cashierContract_Buyer.requestVoucher_ETH_ETH(
        supplyID,
        voucherSupply.voucherOwner,
        {
      ...
      let encodedTopic = await getEncodedTopic(
        receipt,
        VoucherKernel.abi,
        "LogVoucherDelivered"
      );
      data = await decodeData(receipt, encodedTopic, [
        "uint256",
        "address",
        "address",
        "bytes32",
