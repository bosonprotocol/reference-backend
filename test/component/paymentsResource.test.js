...
describe("Payments Resource", () => {
    ...
    context("on GET", () => {
        it("getPaymentsByVoucherTokenId - returns 200 and all payments for the given voucherTokenId", async () => {
            ...
            // CREATE PAYMENT
            const paymentsMetadata = [Random.paymentMetadata({_tokenIdVoucher: voucherTokenId})]; // override with correct id
            const [paymentResponseCode, paymentResponseBody] = await prerequisites.createPayment(token, paymentsMetadata);
            // END CREATE PAYMENT

            const response = await api
                .payments()
                .getByVoucherId(voucherTokenId)
