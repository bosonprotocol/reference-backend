...
const validVoucherStatuses = Object.values(
  require("../../src/utils/voucherStatuses")
);
...
describe("Vouchers Resource", () => {
  ...
  context("on GET", () => {
    it("getAllVouchersForAddress - returns 200 and all vouchers for the given address", async () => {
      ...
      const response = await api.withToken(token).vouchers().getVouchers();
    ...
    it("getVoucherDetails - returns 200 and the requested voucher's details", async () => {
      ...
      const voucherMetadata = prerequisites.createVoucherMetadata(
        voucherSupplyOwner
      ); // override voucher holder to given address
    ...
    it("getAllVouchers - returns 200 and all vouchers", async () => {
      ...
      const response = await api.withToken(gcloudToken).vouchers().getAll();
  ...
  context("on PATCH", () => {
    it("updateVoucherDelivered - returns 200 and updated voucher id", async () => {
      ...
      const data = prerequisites.createVoucherUpdateDeliveredData(
        voucherTokenId,
        voucherIssuer,
        promiseId,
        supplyTokenId,
  ...
  context("on POST", () => {
    it("createVoucher - returns 200 with the voucher ID", async () => {
      ...
      const [
        createVoucherResponseCode,
        createVoucherResponseBody,
      ] = await prerequisites.createVoucher(
        token,
        voucherSupplyId,
        voucherMetadata
