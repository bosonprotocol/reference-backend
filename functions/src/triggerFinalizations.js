...
exports.scheduledKeepersFinalizationsDev = functions.https.onRequest(async (request, response) => {
...
exports.scheduledKeepersFinalizationsDemo = functions.https.onRequest(async (request, response) => {
...
async function triggerFinalizations(executor, config) {
    ...
    for (let i = 0; i < res.data.vouchers.length; i++) {
        ...
        try {
            txOrder = await voucherKernelContractExecutor.triggerFinalizeVoucher(voucherID, {gasLimit: config.GAS_LIMIT});
        ...
        let parsedEvent = await utils.findEventByName(receipt, "LogFinalizeVoucher", "_tokenIdVoucher", "_triggeredBy");
